...
** 特性
   ...
   - middle-keyboard-digit-argument-or-function
     ...
     | Key | function                                                |
     | ,   | 重复 object-catch                                       |
     | .   | 设置 last-goto-point 到当前位置或者将当前所选的内容记录 |
     | SPC | 执行 motion-hint                                        |
     | /   | 反转 object-catch 方向                                  |
...
** 一些特殊操作
   ...
   - 移动
     ...
     - ~(sniem-goto-last-point)~
       一般情况下，跳转到 ~last-point~ 的位置。如果设置了 ~last-goto-point~ ，则跳转到 ~last-goto-point~ 的位置。如果有内容被记录，则跳转到其第一个字符。
       一般情况下优先级:
       被记录的内容 > ~last-goto-point~ > ~last-point~
